...
public class TresEnRaya {
    ...
    public static void main (String [] args){
        ...
        try {
            System.out.print("Selccionar opcion (1-2) o 0 para salir");
...
private static void jugar(Scanner input) {
    ...
    switch (partida (input)) {
        ...
        case charMaquina:
            ...
            break;
        default:
            System.out.println("EMPATE! In");
            break;
    }
}
// Turno del jugador = TRUE / Turno de la maquina = FALSE
...
private static char victoria() (
    ...
    char r=' ':
        do {
            if (charUsuario»tablero(3*1] && charUsuario==tablero[(3*1)+1] 66 charUsuariotablero[(3*1)+2]) {
                r=charUsuario;
            }
            if (charUsuario==tablero[{) && charUsuario==tablero[3+1] 6б charUsuario=tablero{6+£]) ‹
            r=charUsuario;
            }
If(charUsuario tablerofe) & charusuariontablero (4) s charusuario tablerofel)
        }
        if (charusuario=-tablero(2) && charuuario==tablero (4) a charusuario=-tablero 6)) (
r=acharUsuario;
}
        if (charMaquina=«tablero(3*1] 66 chartaquina==tablero[(3+1)+1) 66 charMaquina=«tablero[(3+1)+21) {
r=charMaquina;
}
        if (charMaquina==tablero[1) 66 charMaquina==tablero[3+1] 6б charMaquina=tablero[6+1]) ‹ racharMaquina;
}
        if (charMaquinanntablero(e] 66 chartaquinanntablero[4] 66 charMaquinan«tablero[8]) {
    r=charMaquina;
}
if (charMaquina= tablero(2) & charMaquina==tablero[4) && charMaquina= tablero(6]) € racharMaquina;
}
i+;
        } while (r=' • &$ 1<3);
        return r;
